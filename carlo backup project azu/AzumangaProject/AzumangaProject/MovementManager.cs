...
public static class MovementManager
{
    ...
    public static Boolean isRespawnLegal(int piece, int to, Board board)
    {
        int side = PieceManager.colorOnly(piece);
        ...
        {
            if (PieceManager.colorOnly(piece) == side && board.get1D(to) == Piece.None)
            {
                return true;
            }
        }
    ...
    public static Boolean isMegaLegal(int piece, Move move, Board board)
    {
    ...
    public static void movePiece(Move move, Board board, Boolean _ignoreMagi3_ = false)
    {
        if (move.from < 0)
        {
            int pieceRespawn = -1 * move.from;
            placePiece(pieceRespawn, move.to, board);
            return;
        }
        ...
        int from = move.from;
        int to = move.to;
        if (isMegaLegal(piece, move, board))
        {
    ...
    public static Boolean respawnInterpreter(String comm, Board board)
    {
        ...
        try
        {
            ...
            isValid = isRespawnLegal(Piece.shortNames[fromSTR] | Piece.White, to, board);
            if (isValid)
            {
