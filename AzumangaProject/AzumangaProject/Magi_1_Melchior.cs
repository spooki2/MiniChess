...

{
    ...
    static Boolean rbLegal(int piece, Move move)
    {
        ...
        int deltaX = (Board.to2D(move.to)[0] - Board.to2D(move.from)[0]);
        int deltaY = (Board.to2D(move.to)[1] - Board.to2D(move.from)[1]);


        if (pieceType == Piece.Rook && (deltaX == 0 || deltaY == 0))
        {
            return true;
        }

        if (pieceType == Piece.Bishop && (Math.Abs(deltaX) == Math.Abs(deltaY)))
        {
            return true;
        }
    ...
    public static bool boardWrapCheck(int piece, Move move) //memory intensive funciton
    {
        ...
        */
        ...
        {
            if (offsetMove[0] == deltaX && offsetMove[1] == deltaY)
            {
                return true;
            }
        }
